...
The `tern` R package contains analytical functions for creating tables and graphs used for clinical trials and other statistical analysis. 
...
We could find out that `tern` offers more advanced analitycs with only one line updated in contrast to the pure `rtables` code. 

**More examples with analyze functions are presented in the `tables` vignette.**

### Internals of `tern` Analyze Functions 

**Please skip this subsection if you are not interested in the internals of the analysis function.**

Internally the `tern` analyze functions like `tern::summarize_ancova` are mainly built in the 4 elements chain:
...
The `tern` visualizations are specified with regular function arguments and 
...
We are recommending to apply `tern` outputs in the `teal` package for interactive exploration of data. 
