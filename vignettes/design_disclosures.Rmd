...
Note that `ADSL.STSTFL` does not exist in [STREAM](https://rochewiki.roche.com/confluence/pages/viewpage.action?spaceKey=SPADSCL&title=3.+Macros+and+Programs#id-3.MacrosandPrograms-t_dd_discl) but is generated here. 
...
*Approach 2:* According to [STREAM](https://rochewiki.roche.com/confluence/pages/viewpage.action?spaceKey=SPADSCL&title=3.+Macros+and+Programs#id-3.MacrosandPrograms-t_dd_discl), this table uses `aet07` as a template hence I have demonstrated deaths table production using `adae.AESDTH` as well. Please note that `AESDTH` is the death flag for deaths that were due to AE and will not equal the total number of deaths in the study. I will use approach 2 for production. 
```{r}
...
analyze_term_patients_events <- function(lyt, 
  ...
  analyze_colvars(lyt = lyt, afun = a_term_patients_events_list, extra_args = list(variables = variables, .stats = .stats, ...))
}
...
basic_table() %>%
  split_cols_by_multivar(vars =  c("USUBJID", "AESER", "AEREL", "AESDTH", "AEBODSYS"), 	 
                         varlabels = c("Patients (All)", "Events (All)", "Events (Related)", "Events (Fatal)", "Events (Fatal & Related")) %>%	                         
...
basic_table() %>%
  ...
  analyze_patients_events(.stats = c("patients_all", "events_all")) %>%  # this produces the top row: total n of patients ...
  split_rows_by("AEBODSYS", nested = FALSE) %>%
  analyze_term_patients_events(.stats = c("patients_all", "events_all")) %>%  # this produces all the 2nd level rows below
