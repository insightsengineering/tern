...
```{r s_summary.logical}
s_summary.logical <- function(x, na.rm = TRUE, label = "TRUE", .N_col, .N_row,  #nolint
  ...
  y$count_fraction <- with_label(c(count, ifelse(dn > 0, count / dn, NA)), label)
  y
}
```

```{r s_count_value}
s_count_values <- function(x, values, label = paste(values), na.rm = TRUE, 
                           .N_col, .N_row, denom = c("n", "N_row", "N_col")) {
  UseMethod("s_count_values", x)
}

s_count_values.factor <- function(x, values = "Y", ...) { #nolint
  assert_that(
    is.character(values),
...
s_count_values.character <- function(x, values = "Y", na.rm = TRUE, ...) {
  assert_that(
    is.character(values)
  )
  if (na.rm) x <- x[!is.na(x)]
...
```{r count_values}
count_values <- function(lyt, vars, ..., .stats = "count_fraction") {
  afun <- format_wrap_x(
    s_count_values, 
...
basic_table() %>%
  split_cols_by("ARM", split_fun = add_overall_level("All Patients", first = FALSE)) %>%
  add_colcounts() %>%
  count_values("COMPSTUD", values = "Y", label = "Completed Study") %>%
  split_rows_by("DISSTDFL", split_fun = keep_split_levels("Y")) %>%
  summarize_row_groups(label_fstr = "Discontinued Study") %>%
...
basic_table() %>%
  split_cols_by("ARM", split_fun = add_overall_level("All Patients", first = FALSE)) %>%
  add_colcounts() %>%
  count_values("GOTTRT", values = "Y", label = "Received treatment") %>%
...
Note: Unfortunately, this currently does not work. However I think it should be possible in the future
to do this with rtables. Let's put this on backlog for now and revisit later.
...
basic_table() %>%
  ...
  add_colcounts() %>%
  count_values("COMPSTUD", values = "Y", label = "Completed Study") %>%
  count_values("STUDONS", values = "Alive: In Follow-up", label = "Alive: In Follow-up") %>%
