...
# New plot function.
g_lineplot <- function(df, x = "AVISIT", y = "AVAL", strata = "ARM", # nolint
                       mid = "mean", interval = "mean_ci", whiskers = c("lwr", "upr"), table = NULL,
                       control = control_summarize_vars(), na.rm = TRUE, # nolint
  ...
  df_stats <- summarise(
    ...
    bind_cols(
      lapply(
        s_summary(.data[[y]], control = control, stats = c(mid, interval), na.rm = na.rm),
  ...
  if (!is.null(strata)) {
    ...
    df_N <- df %>% # nolint
      group_by_at(strata) %>%
      summarise(
        N = s_summary(.data[[y]], control = control, stats = "n", na.rm = na.rm)[[1]],
        .groups = "drop"
      )
  ...
  if (!is.null(table)) {
    ...
    df_stats_table <- df_grp %>%
      summarise(
        format_row(
          x = s_summary(.data[[y]], control = control, stats = table, na.rm = na.rm),
          format = summary_numeric_format(),
          labels <- summary_numeric_labels()
        ),
        .groups = "drop"
      )
