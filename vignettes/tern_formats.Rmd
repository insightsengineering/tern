...
Functions that set custom values according to a certain threshold include `format_extreme_values()`, `format_extreme_values_ci()`, and `format_fraction_threshold()`. The extreme value formatters work similarly to allow the user to specify the maximum number of digits to include, and very large or very small values are given a special string value. For example:
...
custom_format <- function(x, ...) {
  ...
  result <- if (x["num"] == 0) {
    ...
    paste0(
      x["num"], "/", x["denom"],
      " (", sprintf("%.3f", round(x["num"] / x["denom"] * 100, 1)), "%)" # We include 3 decimal places by changing %.1f to %.3f
    )
  }
  return(result)
}
