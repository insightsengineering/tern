...
We just take the same as in the `s_mmrm` help page:
...
The model fitting pretty much stays the same as before. We might just rename `s_mmrm` to `fit_mmrm` or similar to follow our new naming convention that we want "expressive verbs" for user facing functions.
...
```{r}
cbind.rtables <- function(...) { # nousage # nolint
  all_args <- list(...)
...
Let's start with the simple tables, that maybe don't need a lot of work in terms of tabulation if the manual tabulation framework still works ok. However, we might consider a more clever generic tabulation function together with a `type` argument to avoid the previous `t_mmrm_*` functions.

```{r}
make_table <- function(x, ...) {
  UseMethod("make_table", x)
}
```
Since `tabulate` and `table` are taken I just came up with this name. But if others have other ideas would be good since this one is not great :-)
...
```{r}
make_table.mmrm <- function(x, type = "fixed") {
  type <- match.arg(type)
  if (type == "fixed") {
    t_mmrm_fixed(x)
  }
...
make_table(mmrm_results, type = "fixed")
```
...
Now we get to the more complicated LS means table. Here we will try to use the approach from [https://pages.github.roche.com/NEST/rtables_examples/model_tabulation.html].
