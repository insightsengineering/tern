...
```{r adae, eval=FALSE}
generate_adae <- function(adsl = ex_adsl,
                        max_n_aes = 5) {
  set.seed(1)
...
```{r adlb, eval=FALSE}
generate_adlb <- function(adsl = ex_adsl,
                        max_n_lbs = 5L) {
  set.seed(1)
...
```{r adpp, eval=FALSE}
generate_adpp <- function(adsl = ex_adsl) {
  set.seed(1)
  paramcd = c(
  ...
  ex_adpp <- adsl %>%
    dplyr::inner_join(adpp, by = c("STUDYID", "USUBJID")) %>%
    dplyr::filter(.data$ACTARM != "B: Placebo", !(.data$ACTARM == "A: Drug X"))

  save(ex_adpp, file = "data/ex_adpp.rda", compress = "xz")
}
```
...
```{r adrs, eval=FALSE}
generate_adrs <- function(adsl = ex_adsl) {
  set.seed(1)
  ...
  save(ex_adrs, file = "data/ex_adrs.rda", compress = "xz")
}
```
...
```{r adtte, eval=FALSE}
generate_adtte <- function(adsl = ex_adsl) {
  set.seed(1)
  ...
  ex_adtte <- dplyr::bind_rows(adtte, data.frame(adtte %>% dplyr::group_by(.data$USUBJID) %>%
  ...
  save(ex_adtte, file = "data/ex_adtte.rda", compress = "xz")
}
```
