...
generate_adlb <- function(adsl = tern_ex_adsl,
  ...
  flag_variables <- function(data,
                             ...
                             apply_filter,
                             apply_mutate) {
    arranged <- dplyr::arrange(grouped, .data$ADTM, .data$ASPID, .data$LBSEQ)

    filtered <- if (apply_filter == TRUE) {
      arranged |>
        dplyr::filter(
      ...
      (
       function(d) {
         if (apply_mutate == TRUE) {
           dplyr::mutate(d, new_var = ifelse(is.na(.data$DTYPE), "Y", ""))
         } else {
           dplyr::mutate(d, new_var = ifelse(is.na(.data$AVAL) == FALSE & is.na(.data$DTYPE), "Y", ""))
         }
       })() |>
      dplyr::ungroup()
  ...
  adrs <- split(adsl, adsl$USUBJID) |>
    ...
    (\(x) {
      dplyr::mutate(
        ...
        DTHFL = factor(sample(c("Y", "N"), nrow(x), replace = TRUE, prob = c(1, 0.8)))
      )
    })()
