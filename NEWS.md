# tern 0.7.4.9000
* Added a couple of new statistics to `s_summary.numeric`. Added `names` attribute to each and every element of the final list returned by the `s_summary.numeric` function. Added `summary_numeric_format` and `summary_numeric_labels` simple functions in favor of existing `.a_summary_numeric_formats` and `.a_summary_numeric_labels` variables in `a_summary` function.
