...
### Enhancements
* Started deprecation cycle for `summarize_vars` and `control_summarize_vars` names by using aliases. Renamed into `analyze_vars` and `control_analyze_vars` to reflect underlying `rtables`machinery.
