# tern 0.7.4.9000
* Parametrized `s_summary.numeric` so that only requested statistics are computed and returned. Added a couple of new statistics. Added `names` attribute to each and every element of the final list returned by the `s_summary.numeric` function.
