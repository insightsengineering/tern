...
#'
#' The set of variables given in `variables` is complemented to the set of variables generated by
#' [tern::control_lineplot_vars()] function, i.e. if `g_lineplot` is invoked with e.g.
#' `variables = c(strata = "ARMCD")`, then `variables` will be extended by all the variables
#' (other than `strata`), as returned by [tern::control_lineplot_vars()].
...
#' g_lineplot(adlb, variables = c(strata = NA))
#'
...
g_lineplot <- function(df, # nolint
  ...
  variables_default <- control_lineplot_vars()
  variables <- c(variables, variables_default[setdiff(names(variables_default), names(variables))])
...
#' @export
#'
control_lineplot_vars <- function() {
  c(x = "AVISIT", y = "AVAL", strata = "ARM", paramcd = "PARAMCD", y_unit = "AVALU")
}
