...
#' @param var_names (`character`)\cr names of each variable/level corresponding to pair of proportions in
...
#' @export
#' @examples
#' stat_risk_diff_ci(frac_x = list(0.375), frac_y = list(0.01), N_x = 5, N_y = 5, var_names = c("x"), conf_level = 0.9)
#' stat_risk_diff_ci(frac_x = list(0.5, 0.75, 1), frac_y = list(0.25, 0.05, 0.5), N_x = 10, N_y = 20, pct = FALSE)
stat_risk_diff_ci <- function(frac_x, frac_y, N_x, N_y, conf_level = 0.95, var_names = NULL, pct = TRUE) {
  checkmate::assert_list(frac_x, types = "numeric")
  checkmate::assert_list(frac_y, types = "numeric", len = length(frac_x))
  checkmate::assert_character(var_names, len = length(frac_x), null.ok = TRUE)
