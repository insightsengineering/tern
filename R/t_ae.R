...
#' 
#' 
lt_ae_max_grade_d1 <- function(class, 
                               id, 
                               grade, 
                               col_by, 
                               col_N = tapply(col_by, col_by, length),
  ...
  # class chunk
    ...
    l_t_class <- Map(function(df_class, class_i) {
      
      tbl_raw <- t_max_grade_per_id(
        grade = df_class$grade,
        id = df_class$id,
        col_by = df_class$col_by,
        col_N = col_N,
        grade_levels = grade_levels,
        any_grade = "- Any Grade -"
      )
...
lt_ae_count_d2 <- function(class, 
  ...
  l_t_class_terms <- Map(function(df_cl, class_i) {
    ...
    df_by_term <- c(
      list("Total number of patients with at least one adverse event" = df_cl),
      list("Total number of events" = df_cl),
      split(df_cl, df_cl$term)      
    )
    
    l_t_terms <- Map(function(df_term, term_i, d_i, p_i) {
      
      tbl <- t_helper_tabulate( df_term, col_N, " ", term_i, d_i, p_i)
    ...
    names(df_by_term), 
    c(TRUE, FALSE, rep(TRUE, length(df_by_term)-2)),  
    c(TRUE, FALSE, rep(TRUE, length(df_by_term)-2)))
