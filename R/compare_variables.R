...
#'
#' The analyze function [compare_vars()] creates a layout element to generate a summary and comparison of one or more
#' variables, using the S3 generic function [s_summary()] to calculate a list of summary statistics. A list of all
#' available statistics for numeric variables can be viewed by running
#' `get_stats("analyze_vars_numeric", add_pval = TRUE)` and for non-numeric variables by running
#' `get_stats("analyze_vars_counts", add_pval = TRUE)`. Use the `.stats` parameter to specify the statistics to include
#' in your output summary table.
#'
