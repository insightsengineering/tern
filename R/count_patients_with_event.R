...
s_count_patients_with_event <- function(df,
  ...
  if (length(col_names) == 1) {
    var_label <- obj_label(df[[col_names]])

    if (!is.null(var_label)) {
      result <- list(
        n = with_label(result$n, var_label),
        count = with_label(result$count, var_label),
        count_fraction = with_label(result$count_fraction, var_label)
      )
    }
  }
...
#'   used for counting the number of unique identifiers. The flag variable labels are used as
#'   row labels in table generated by the layout.
#'
#' @export
#' @examples
#'
...
count_patients_with_flags <- function(lyt,
  ...
  for (i in seq_along(flag_variables)) {
    ...
    flag_var <- unname(flag_variables[i])
