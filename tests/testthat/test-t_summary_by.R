...
ADSL <- read.table(header = TRUE, sep = ';', stringsAsFactors = FALSE, text = '
...
ADSL$ARMCD <- factor(ADSL$ARMCD, levels = c("ARM A", "ARM B", "ARM C"))
ADSL$COUNTRY <- factor(ADSL$COUNTRY, levels = c("AFG", "BES", "CUW", "SXM"))

attributes(ADSL$USUBJID)$label <- "Subject ID"
attributes(ADSL$AGE)$label <- "Age"
attributes(ADSL$SEX)$label <- "Sex"
attributes(ADSL$ARMCD)$label <- "Arm Code"
attributes(ADSL$COUNTRY)$label <- "Country"
