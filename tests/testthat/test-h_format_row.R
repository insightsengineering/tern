testthat::test_that("h_format_row returns the correct dataframe", {
  ...
  result <- tern::h_format_row(x, format, labels)
  expected <- data.frame("My Mean" = "50.0", V1 = "(48.00, 51.00)", stringsAsFactors = FALSE, check.names = FALSE)
  ...
  result <- tern::h_format_row(x, format, labels)
  expected <- data.frame(
    "My Mean" = "50.0", "Mean 95% CI" = "(48.00, 51.00)",
