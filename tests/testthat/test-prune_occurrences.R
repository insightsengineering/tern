...
test_that("has_count_in_cols result works in a special case identical to standard pruning", {
  tab <- get_table()
  row_condition <- has_count_in_cols(above = 0L, col_names = names(tab))
...
test_that("has_count_in_cols result performs comparisons correctly", {
  ...
  row_condition <- has_count_in_cols(above = 3L, col_names = "A: Drug X")
  expect_false(row_condition(table_row))
  row_condition <- has_count_in_cols(above = 0L, col_names = "A: Drug X")
  expect_true(row_condition(table_row))
  row_condition <- has_count_in_cols(above = 4L, col_names = c("A: Drug X", "B: Placebo"))
  expect_true(row_condition(table_row))
})

test_that("has_fraction_in_cols result works in a special case identical to standard pruning", {
  tab <- get_table()
  row_condition <- has_fraction_in_cols(above = 0, col_names = names(tab))
  result <- prune_table(tab, keep_rows(row_condition))
  expected <- prune_table(tab)
  expect_identical(result, expected)
})
...
test_that("has_fraction_in_cols result performs comparisons correctly", {
  ...
  table_row <- collect_leaves(sub_tab)[[1]]
  row_condition <- has_fraction_in_cols(above = 0.01, col_names = "A: Drug X")
  expect_false(row_condition(table_row))
  row_condition <- has_fraction_in_cols(above = 0.008, col_names = "A: Drug X")
  expect_true(row_condition(table_row))
  row_condition <- has_fraction_in_cols(above = 0.02, col_names = c("A: Drug X", "B: Placebo"))
  expect_true(row_condition(table_row))
})
...
test_that("has_fractions_difference result performs comparisons correctly", {
  ...
  table_row <- collect_leaves(sub_tab)[[1]]
  row_condition <- has_fractions_difference(above = 0.01, col_names = c("A: Drug X", "B: Placebo"))
  expect_true(row_condition(table_row))
  row_condition <- has_fractions_difference(above = 0.5, col_names = c("A: Drug X", "B: Placebo"))
  expect_false(row_condition(table_row))
  row_condition <- has_fractions_difference(above = 0.01, col_names = c("A: Drug X", "C: Combination"))
  expect_false(row_condition(table_row))
})
...
test_that("has_counts_difference result performs comparisons correctly", {
  ...
  table_row <- collect_leaves(sub_tab)[[1]]
  row_condition <- has_counts_difference(above = 2L, col_names = c("A: Drug X", "B: Placebo"))
  expect_true(row_condition(table_row))
  row_condition <- has_counts_difference(above = 3L, col_names = c("A: Drug X", "B: Placebo"))
  expect_false(row_condition(table_row))
  row_condition <- has_counts_difference(above = 0L, col_names = c("A: Drug X", "C: Combination"))
  expect_false(row_condition(table_row))
})
...
test_that("combination of pruning functions works", {
  ...
  result <- tab %>%
    prune_table(
      keep_rows(
        has_fractions_difference(above = 0.1, c("A: Drug X", "C: Combination")) &
          has_counts_difference(above = 1L, c("B: Placebo", "A: Drug X")) &
          has_count_in_cols(above = 2L, "A: Drug X")
      )
    )
