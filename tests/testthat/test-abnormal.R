...
test_that("count_abnormal also works with tibble and custom arguments", {
  df <- dplyr::as_tibble(
    data.frame(
...
test_that("s_count_abnormal works as expected with healthy input", {
  df <- dplyr::as_tibble(
    data.frame(
      myid = as.character(c(1, 1, 1, 2, 2, 2)),
      myvisit = factor(c("SCREENING", "BASELINE", "WEEK 1", "SCREENING", "BASELINE", "WEEK 1")),
...
test_that("analyze_abnormal works with default arguments", {
  df <- data.frame(
    USUBJID = as.character(c(1, 1, 2, 2)),
    AVISIT = factor(c("BASELINE", "WEEK 1", "BASELINE", "WEEK 1")),
    ANRIND = factor(c("NORMAL", "LOW", "HIGH", "HIGH"))
...
test_that("analyze_abnormal works with custom arguments", {
  df2 <- data.frame(
...
test_that("analyze_abnormal works with more than one variable in `vars`", {
  df2 <- data.frame(
    ID = as.character(c(1, 1, 2, 2)),
    VISIT = factor(c("SCREENING", "WEEK 1", "SCREENING", "WEEK 1")),
    RANGE = factor(c("NORMAL", "LOW", "HIGH", "HIGH")),
    RANGE2 = factor(c("LOW", "HIGH", "NORMAL", "HIGH"))
  )
  result <- basic_table() %>%
    analyze_abnormal(
      vars = c("RANGE", "RANGE2"),
