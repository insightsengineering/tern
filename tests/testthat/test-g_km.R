...
testthat::test_that("g_km plot with titles/footnotes and custom group labels works", {
  df <- df %>%
    dplyr::filter(ARM == "A: Drug X") %>%
    dplyr::mutate(group = as.factor(ifelse(AGE > 34, ">Median", "<=Median")))

  variables <- list(tte = "AVAL", is_event = "is_event", arm = "group")

  g_km_tf_cust_lbls <- withr::with_options(
    opts_partial_match_old,
    g_km(
      df = df,
      variables = variables,
  ...
  expect_snapshot_ggplot("g_km_tf_cust_lbls", g_km_tf_cust_lbls, width = 8, height = 5)
})
...
testthat::test_that("g_km ylim parameter works as expected", {
  ...
  g_km_custom_ylim_failure <- withr::with_options(
    opts_partial_match_old,
    g_km(
      df = df,
      variables = variables,
  ...
  expect_snapshot_ggplot("g_km_custom_ylim_failure", g_km_custom_ylim_failure, width = 8, height = 4)
})
...
testthat::test_that("ref_group_coxph parameter works as expected", {
  g_km_ref_group_coxph <- withr::with_options(
    ...
    g_km(
      ...
      control_annot_coxph = control_coxph_annot(x = 0.25, y = 0.25, w = 0.35, ref_lbls = TRUE)
    )
  )
  expect_snapshot_ggplot("g_km_ref_group_coxph", g_km_ref_group_coxph, width = 10, height = 5)
})
