...
```{r}
h_toxicity_grade_group <- function(df,
...
h_toxicity_grade_map <- function(paramcds = c("ALKPHSI", "GLUCSI"),
  ...
  for (paramcd in paramcds) {
    ...
    if (length(paramcd_df) == 1) {
      paramcd_df <- expand.grid(a, b)
    } else if (length(paramcd_df) == 2) {
      paramcd_df <- expand.grid(a, b, b)
    }
