...
adlb_f <- adlb %>%
    ...
    dplyr::mutate(
      ...
      WGRLOFL = dplyr::case_when(WGRLOFL == "Y" ~ TRUE, TRUE ~ FALSE),
      WGRHIFL = dplyr::case_when(WGRHIFL == "Y" ~ TRUE, TRUE ~ FALSE)
    ) %>%
    droplevels()
...
s_count_abnormal_by_worst_grade <- function(df, #nolint
  ...
  abn_levels <-  if (max_atoxgrade_num > 0 & min_atoxgrade_num < 0) {
     c(low_label, high_label)
      ...
      high_label
      } else if (min_atoxgrade_num < 0 & max_atoxgrade_num <= 0) {
      low_label
      } else NULL

   df <- df[!is.na(df[[.var]]), ]
   if (is.null(abn_levels)) {
     result <- list(empty = NULL)
   } else {
...
s_count_abnormal_by_worst_grade(
   df = adlb_f2 %>% dplyr::filter(ARMCD == "ARM A" & PARAMCD == "ALT") %>% droplevels(),
   .var = "ATOXGR",
   variables = list(id = "USUBJID", worst_low_flag = c(Looooow = "WGRLOFL"), worst_high_flag = c(High = "WGRHIFL")))

s_count_abnormal_by_worst_grade(
   df = adlb_f2 %>% dplyr::filter(ARMCD == "ARM A" & PARAMCD == "IGA") %>% droplevels(),
   .var = "ATOXGR",
   variables = list(
   id = "USUBJID",
   worst_low_flag = c(Looooow = "WGRLOFL"),
   worst_high_flag = c(Hiiiiiigh = "WGRHIFL")
   )
  )

s_count_abnormal_by_worst_grade(
   df = adlb_f2 %>% dplyr::filter(ARMCD == "ARM A" & PARAMCD == "CRP") %>% droplevels(),
   .var = "ATOXGR",
...
s_count_abnormal_by_worst_grade(
   ...
   variables = list(
   id = "USUBJID",
   worst_low_flag = c(Low = "WGRLOOOOFL"),
   worst_high_flag = c("Very High" = "WGRHIFL")
   )
  )
...
* Example: Case where there is an arm that has no lab records.

Example 1: Case where all `ATOXGRADE` for ALT parameter are 0.
```{r}
adlb_f3 <- adlb_f2 %>%
  dplyr::filter(PARAMCD != "ALT" | ATOXGR == 0)

adlb_f3 %>% dplyr::count(PARAMCD, ATOXGR)
...
empirical_map1 <- map %>% dplyr::filter(
...
```{r}
empirical_map2 <- map %>% dplyr::filter(
  PARAMCD != "ALT"
)
...
Another option would be to trim 0 rows that do not contain "-Any" as the rowname.
...
result <- basic_table() %>%
  ...
  append_topleft("  Direction of Abnormality") %>%
  append_topleft("    Highest NCI CTCAE Grade") %>%
  build_table(df = adlb_f3, alt_counts_df = adsl)


all_zero_or_na_not_any <- function(tr) {
  is_any_row <- length(grep("+Any$", obj_label(tr), value = TRUE)) > 0
  if (!is(tr, "TableRow") || is(tr, "LabelRow") || is_any_row)
    return(FALSE)
...
```{r}
adlb_f4 <- adlb_f2 %>%
  dplyr::filter(PARAMCD != "ALT" | as.numeric(as.character(ATOXGR)) < 0)
adlb_f4 %>% dplyr::count(PARAMCD, ATOXGR)
```
...
Example 4:  Case where there is an arm that has no lab records.

```{r}
adlb_f3 <- adlb_f2 %>% dplyr::filter(!(ARMCD == "ARM A" & PARAMCD == "ALT"))
