...
```{r}
#just checking if the conversions are properly performed. 
atox1 <- adlb$ATOXGR
head(atox1)
atox2 <- as.numeric(as.character(adlb$ATOXGR))
head(atox2)
atox3 <- as.factor(atox2)
head(atox3)
...
s_count_abnormal_by_worst_grade <- function(df, #nolint
                                            ...
                                            variables = list(
                                              id = "USUBJID", 
                                              worst_grade_flag = c(Low = "WGRLOFL", High = "WGRHIFL")
                                              )
                                            ) {
  ...
  assert_that(
    ...
    all(variables$worst_grade_flag %in% c("WGRLOFL", "WGRHIFL")),
    is_df_with_variables(df, c(aval = .var, variables)),
    is.factor(df[[.var]]),
  ...
  low_label <- names(variables$worst_grade_flag)[variables$worst_grade_flag == "WGRLOFL"]
  high_label <- names(variables$worst_grade_flag)[variables$worst_grade_flag == "WGRHIFL"]
    ...
    result <- list()
     ...
     abn_levels <-  if (max_atoxgrade_num > 0 & min_atoxgrade_num < 0) { 
        c(low_label, high_label)
         ...
         #c(abnormal[names(abnormal) == "High"])
         high_label
         } else { 
         low_label
         }
      df <- df[!is.na(df[[.var]]), ]
      flag_low_name <- variables$worst_grade_flag[names(variables$worst_grade_flag) == low_label]
      flag_high_name <- variables$worst_grade_flag[names(variables$worst_grade_flag) == high_label]
      ...
      # Denominator is number of patients with at least one valid measurement during treatment
        ...
        for (abn in abn_levels){
          abnormal <- abn
          #abnormal_label <- abn_levels[names(abn_levels) == abnormal]
          name_any <- paste(abnormal, "- Any")
          if (abnormal == low_label){
            anl_abn <- anl[anl$flag_low & as.numeric(as.character(anl$grade)) < 0, , drop = FALSE]
            grades <- sort(as.numeric(levels(anl_abn$grade)[levels(anl_abn$grade) < 0]), decreasing = TRUE)
          ...
          for (grade in grades) {
            ...
            result[[abn_grade]] <- with_label(c(num, num / n), abn_grade)
          }
          any_grade_num <- length(unique(anl_abn$id))
          result[[name_any]] <- with_label(c(any_grade_num , any_grade_num/n), name_any)
        }
  }
...
s_count_abnormal_by_worst_grade(
   ...
   .var = "ATOXGR",
   variables = list(id = "USUBJID", worst_grade_flag = c(Looooow = "WGRLOFL", High = "WGRHIFL"))
 )

s_count_abnormal_by_worst_grade(
   df = adlb_f2 %>% filter(ARMCD == "ARM A" & PARAMCD == "IGA") %>% droplevels(),
   .var = "ATOXGR",
   variables = list(id = "USUBJID", worst_grade_flag = c(Low = "WGRLOFL", "Very High" = "WGRHIFL"))
 )

s_count_abnormal_by_worst_grade(
   df = adlb_f2 %>% filter(ARMCD == "ARM A" & PARAMCD == "CRP") %>% droplevels(),
   .var = "ATOXGR",
   variables = list(id = "USUBJID", worst_grade_flag = c("Very Low" = "WGRLOFL", "Very High" = "WGRHIFL"))
 )

#if something different from "WGRLOFL"/"WGRHIFL" is introduced as worst grade flags, the function will return an error. 

s_count_abnormal_by_worst_grade(
   df = adlb_f2 %>% filter(ARMCD == "ARM A" & PARAMCD == "CRP") %>% droplevels(),
   .var = "ATOXGR",
   variables = list(id = "USUBJID", worst_grade_flag = c(Low = "WGRLOFLLLLL", High = "WGRHIFL"))
 )
 ...
 * Layout without new split function,  correctly. 
...
lyt_no_trim <- basic_table() %>%
  ...
  count_abnormal_by_worst_grade(
    var = "ATOXGR",
    variables = list(id = "USUBJID", worst_grade_flag = c(Loooooow = "WGRLOFL", High = "WGRHIFL"))
  ) %>%
  append_topleft("  Direction of Abnormality") %>%
  append_topleft("    Highest NCI CTCAE Grade")
...
Example 3: All grades in an opposite direction from stated in map. 
Scenario where map is incorrectly filled.
It is returning an error that theoretically has nothing to do with this. 
It works with `map = map` but not with `map2`.

```{r}
map2 <- map
map2[map2$PARAMCD == "ALT", 2] <- "1"
