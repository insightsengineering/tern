...
adlb_labels <- formatable::var_labels(adlb)
...
formatable::var_labels(adlb_f) <- adlb_labels
```
...
s_count_abnormal_by_worst_grade <- function(df, # nolint
  ...
  checkmate::assert_logical(df[[variables$worst_grade_flag]], min.len = 1, any.missing = FALSE)
...
s_count_abnormal_by_worst_grade <- function(df, # nolint
  ...
  checkmate::assert_logical(df[[variables$worst_low_flag]], any.missing = FALSE)
  checkmate::assert_logical(df[[variables$worst_high_flag]], any.missing = FALSE)
  ...
  if (is.null(abn_levels)) {
    ...
    for (abn in abn_levels) {
      ...
      for (grade in grades) {
        ...
        result[[abn_grade]] <- formatable::with_label(c(num, fraction), abn_grade)
      }
      any_grade_num <- length(unique(anl_abn$id))
      fraction_any <- if_else(n == 0, 0, any_grade_num / n)
      result[[name_any]] <- formatable::with_label(c(any_grade_num, fraction_any), name_any)
    }
...
lyt_trim_map <- basic_table() %>%
  ...
  split_rows_by("PARAMCD",
    label_pos = "topleft", split_label = obj_label(adlb_f2$PARAMCD),
...
```{r}
adlb_f3 <- adlb_f2 %>% filter(!(ARMCD == "ARM A" & PARAMCD == "ALT"))
