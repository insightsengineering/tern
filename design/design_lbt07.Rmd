...
s_count_abnormal_by_worst_grade <- function(df, #nolint
  ...
  assert_that(
    ...
    all(names(variables$worst_grade_flag) %in% c("Low", "High")),
    is_df_with_variables(df, c(aval = .var, variables)),
    is.factor(df[[.var]]),
    ...
    result <- list()
     ...
     abn_levels <-  if (max_atoxgrade_num > 0 & min_atoxgrade_num < 0) { 
       abnormal
         ...
         } else c(abnormal[names(abnormal) == "Low"])
      ...
      # Denominator is number of patients with at least one valid measurement during treatment
        ...
        for (abn in names(abn_levels)){
          ...
          name_any <- paste(abnormal_label, "- Any")
          if (abnormal == "Low"){
          ...
          for (grade in grades) {
            ...
            abn_grade <- paste(abnormal_label, "-", abs(as.numeric(grade)))
...
#The `abnormal` argument is now only going to be used for selecting the labels for "LOW"/"HIGH". 
#If we want them to keep always as "Low"/"High", this argument could be removed. 
#See the following example 

s_count_abnormal_by_worst_grade(
   df = adlb_f2 %>% filter(ARMCD == "ARM A" & PARAMCD == "ALT") %>% droplevels(),
   .var = "ATOXGR",
   abnormal = c(Low = "Loooooooow", High = "High"),
   variables = list(id = "USUBJID", worst_grade_flag = c("WGRLOFL", "WGRHIFL"))
 )
