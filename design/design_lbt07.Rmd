...
adlb_labels <- formatters::var_labels(adlb)
...
formatters::var_labels(adlb_f) <- adlb_labels
```
  ...
  if (is.null(abn_levels)) {
    ...
    for (abn in abn_levels) {
      ...
      for (grade in grades) {
        ...
        result[[abn_grade]] <- formatters::with_label(c(num, fraction), abn_grade)
      }
      any_grade_num <- length(unique(anl_abn$id))
      fraction_any <- if_else(n == 0, 0, any_grade_num / n)
      result[[name_any]] <- formatters::with_label(c(any_grade_num, fraction_any), name_any)
    }
