...
As we have seen above, `ATOXGRADE` variable is converted to numeric as the statistics function was designed to work with numeric `ATOXGRADE`. However, our goal now within the statistics function is to be able to infer the target directions to be displayed by usiong `ATOXGRADE` and we need to keep it as a factor in order to retain the levels and be still able to infer in edge cases where we may not have any abnormality grade for a certain parameter. 

First, let's go reconvert `ATOXGRADE` back to factor. 

```{r}
#just checking if the conversions are working properly
atox1 <- adlb$ATOXGR
head(atox1)
...
s_count_abnormal_by_worst_grade <- function(df, #nolint
                                            .var = "ATOXGR",
                                            abnormal = c("LOW", "HIGH"), #we may not need this anymore! 
                                            variables = list(
                                              id = "USUBJID", 
 ...
 abn_levels <-  if (max_atoxgrade_num > 0 & min_atoxgrade_num < 0) { 
  ...
  df$atoxgrade_num <- as.numeric(as.character(df[[.var]]))
  df$abn_grade <- factor(ifelse(df$atoxgrade_num < 0, paste("Low -", abs(df$atoxgrade_num)), 
                                                     ifelse(df$atoxgrade_num > 0, paste("High -", abs(df$atoxgrade_num)),
                                                            paste("Normal -", abs(df$atoxgrade_num)))))

  anl <- data.frame(
      id = df[[variables$id]],
      grade = as.numeric(as.character(df[[.var]])),
      abn_grade = df$abn_grade,
  ...
  # Denominator is number of patients with at least one valid measurement during treatment
    ...
    for (abn in abn_levels){
      ...
      #abnormal_grades <- unique(anl_abn$abn_grade)
      abnormal_grades <- levels(droplevels(anl_abn$abn_grade))
      for (abn_grade in abnormal_grades) {
        num <- length(unique(anl_abn[anl_abn$abn_grade == abn_grade, "id", drop = TRUE]))
        result[[abn_grade]] <- with_label(c(num, num / n), as.character(abn_grade))
      }
...
s_count_abnormal_by_worst_grade(
   ...
   .var = "ATOXGR",
   abnormal = c("LOW", "HIGH"),
   variables = list(id = "USUBJID", worst_grade_flag = c("WGRLOFL", "WGRHIFL"))
 )
...
map <- data.frame(
  ...
  ATOXGR = as.character(c(seq(0,4), c(seq(-4,4)), seq(0,4))),
  stringsAsFactors = FALSE
)
...
## Edge cases
* All grades 0 for a param. 
* All grades in the target direction (none 0). 
* All grades in an opposite direction 
* All grades for all params 0. 
