...
s_count_abnormal_by_worst_grade <- function(df, #nolint
  ...
  assert_that(
    ...
    is_df_with_variables(df, c(aval = .var, variables)),
    is.factor(df[[.var]]),
    is_logical_vector(df[[variables$worst_low_flag]]) | is.logical(df[[variables$worst_low_flag]]),
    is_logical_vector(df[[variables$worst_high_flag]]) | is.logical(df[[variables$worst_high_flag]]),
    is_character_or_factor(df[[variables$id]])
  )
