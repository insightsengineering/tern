...
s_count_abnormal_by_worst_grade <- function(df = adlb_f, #nolint
  ...
  assertthat::assert_that(
    ...
    assertthat::is.string(variables$param),
    assertthat::is.string(variables$grade_dir),
  ...
  assertthat::assert_that(all(c(variables[["param"]], variables[["grade_dir"]]) %in% .spl_context$split),
...
s_count_abnormal_by_worst_grade(
   df = adlb_f %>% dplyr::filter(ARMCD == "ARM A" & PARAMCD == "ALT") %>% droplevels(),
...
build_table(
  lyt = lyt_no_trim,
  df = adlb_f %>% dplyr::filter(WGRLOFL == TRUE | WGRHIFL == TRUE),
  alt_counts_df = adsl
)
...
build_table(
  lyt = lyt_trim_map2,
  df = adlb_f_no_ALT %>% dplyr::filter(WGRLOFL == TRUE | WGRHIFL == TRUE),
  alt_counts_df = adsl
)
```
...
adlb_f2 %>% dplyr::count(PARAMCD, ATOXGR, GRADE_ANL)
```

```{r}
build_table(
  lyt = lyt_trim_map,
  df = adlb_f2 %>% dplyr::filter(WGRLOFL == TRUE | WGRHIFL == TRUE),
  alt_counts_df = adsl
)
```
...
build_table(
  lyt = lyt_trim_map,
  df = adlb_f3 %>% dplyr::filter(WGRLOFL == TRUE | WGRHIFL == TRUE),
  alt_counts_df = adsl
)
```
