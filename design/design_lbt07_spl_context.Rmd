...
s_count_abnormal_by_worst_grade <- function(df = adlb_f, #nolint
  ...
  param <- .spl_context$split[.spl_context$split %in% variables[["param"]]]
  
  assert_that(
    is.string(.var),
    is.list(variables),
    is.string(variables$id),
    is_character_vector(variables$param),
    ...
    is.factor(df[[param]]),
    all(!is.na(df[[.var]])), #NA-s are not accepted in Grades
    #To verify that the split_rows_by are performed with correct variables.
    all(c(param, variables[["anrind"]]) %in% .spl_context$split)
    )

  # I am adding variables$param = c("PARAMCD", "PARAM")
  #as the user can split_rows_by either PARAMCD or PARAM
  first_row <- .spl_context[.spl_context$split %in% variables[["param"]], ]
  param <- first_row$split
  ...
  denom <- subj_cur_col %>% unique() %>% length()
...
s_count_abnormal_by_worst_grade(
   ...
   .spl_context = data.frame(cur_col_n = c(0, 0), split = c("PARAM", "GRADE_DIR")),
   .var = "GRADE_ANL",
   variables = list(id = "USUBJID", param  = c("PARAMCD", "PARAM"),  anrind = c("GRADE_DIR")))
...
lyt_no_trim <- basic_table() %>%
  ...
  split_rows_by("GRADE_DIR", label_pos = "topleft", split_label = "Direction of abnormality", split_fun = remove_split_levels("NORMAL")) %>%
...
lyt_trim_map <- basic_table() %>%
  ...
  split_rows_by("GRADE_DIR", label_pos = "topleft", split_label = "Direction of abnormality", split_fun = trim_levels_to_map(map = map)) %>%
  count_abnormal_by_worst_grade(
    var = "GRADE_ANL"
  ) %>%
  append_topleft("    Highest NCI CTCAE Grade")
...
lyt_trim_group <- basic_table() %>%
  ...
  split_rows_by("GRADE_DIR", label_pos = "topleft",
...
lyt_trim_map2 <- basic_table() %>%
  ...
  split_rows_by("GRADE_DIR", label_pos = "topleft", split_label = "Direction of abnormality", split_fun = trim_levels_to_map(map = empirical_map2)) %>%
  count_abnormal_by_worst_grade(
    var = "GRADE_ANL"
  ) %>%
  append_topleft("    Highest NCI CTCAE Grade")
...
build_table(
  ...
  df = adlb_f %>% filter(WGRLOFL == TRUE | WGRHIFL == TRUE),
  alt_counts_df = adsl
) #We are obtaining an error.

# Let's remove ALT records from the analysis dataset.
# It's seems we have no other choice but to exclude ALT from dataset as well.

adlb_f_no_alt <- adlb_f %>% filter(PARAMCD != "ALT") %>% droplevels()
...
lyt_trim_map3 <- basic_table() %>%
  ...
  split_rows_by("GRADE_DIR", label_pos = "topleft", split_label = "Direction of abnormality", split_fun = trim_levels_to_map(map = map2)) %>%
  count_abnormal_by_worst_grade(
    var = "GRADE_ANL"
  ) %>%
  append_topleft("    Highest NCI CTCAE Grade")
