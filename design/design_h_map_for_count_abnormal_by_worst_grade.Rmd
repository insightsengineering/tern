...
adsl <- ADSL %>% dplyr::filter(SAFFL == "Y") %>% df_explicit_na()
adlb <- ADLB %>% df_explicit_na() %>%
  dplyr::filter(ONTRTFL == "Y" & ATOXGR != "<Missing>" & PARCAT2 == "SI" & SAFFL == "Y")
adlb_f <- adlb %>%
  dplyr::mutate(
...
# map <- adlb_f %>% dplyr::select(PARAMCD, GRADE_DIR, GRADE_ANL) %>% unique() %>% dplyr::filter(GRADE_DIR != "ZERO")
# map <- ext_ncictc4 %>% dplyr::left_join(map, by = c("PARAMCD", "GRADE_DIR")) %>% unique() %>% dplyr::arrange(PARAMCD)
# map <- map %>% dplyr::mutate(GRADE_ANL = ifelse(is.na(GRADE_ANL), "Any", as.character(GRADE_ANL)))
