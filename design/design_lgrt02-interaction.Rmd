...
adrs_labels <- var_labels(ADRS) #nolint
...
h_interaction_coef_name <- function(
  interaction_vars,
...
h_odds_ratio_categorical_interaction <- function( #nolint
...
```{r}
h_odds_ratio_continuous_interaction <- function( #nolint
  odds_ratio_var,  # name of the variable for which to calculate odds ratios
  interaction_var,   # name of the interaction variable
  fit_glm,
  ...
  assert_that(
    ...
    all(c(odds_ratio_var, interaction_var) %in% interaction_vars),
    identical(length(interaction_vars), 2L),
    is.null(interaction_increments) || is_numeric_vector(interaction_increments)
  )
  xs_level <- fit_glm$xlevels
  xs_coef <- coef(fit_glm)
  xs_vcov <- vcov(fit_glm)
...
```{r}
h_odds_ratio_interaction <- function(
  odds_ratio_var,  # name of the variable for which to calculate odds ratios
  interaction_var,   # name of the interaction variable
  fit_glm,
  conf_level = 0.95,
...
```{r}
glm_inter_term_extract <- function(odds_ratio_var,
                                   interaction_var,
                                   fit_glm,
                                   ...) {
...
```{r}
h_logistic_inter_terms <- function(x, fit_glm, conf_level, 
                                   interaction_increments = NULL) {
...
```{r logistic_cols}
logistic_cols <- function(lyt, conf_level = 0.95) {
...
ident_cfun <- function(var, flag, format = "xx") {
  force(var)
  force(format)
