...
Note quantile_type is set to Type 3 as this matches SAS definition but this needs to be checked.
...
s_summary.numeric <- function(x,
  ...
  y$mean_ci <- with_label(mean_ci, paste("Mean", tern:::f_conf_level(conf_level)))

  median_ci_df <- stat_median_ci(x, conf_level = conf_level, na.rm = na.rm)
  median_ci <- unname(as.numeric(median_ci_df[, c("ymin", "ymax")]))
  y$median_ci <- with_label(median_ci, paste("Median", tern:::f_conf_level(conf_level)))
