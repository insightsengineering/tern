...
s_summary.numeric <- function(x,
  ...
  y$quantiles <- formatters::with_label(unname(qts), paste0(quantiles[1] * 100, "% and ", quantiles[2] * 100, "%-ile"))

  y$range <- range_noinf(x)

  mean_ci <- stat_mean_ci(x, conf_level = conf_level, na.rm = na.rm)
  y$mean_ci <- formatters::with_label(mean_ci, paste("Mean", tern:::f_conf_level(conf_level)))
  ...
  y$median_ci <- formatters::with_label(median_ci, paste("Median", tern:::f_conf_level(conf_level)))
