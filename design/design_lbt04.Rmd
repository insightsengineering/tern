...
adlb_no_CRP_L <- dplyr::filter(adlb, !(PARAMCD == "CRP" & ANRIND == "LOW")) # "0" in single row only CRP -> LOW # nolintr
adlb_no_ALT_LN <- dplyr::filter(adlb, !(PARAMCD == "ALT" & ANRIND %in% c("LOW", "NORMAL"))) # "0" for the whole ALT inner group # nolintr
adlb_no_IGA_NH <- dplyr::filter(adlb, !(PARAMCD == "IGA" & ANRIND %in% c("HIGH", "NORMAL"))) # "0" for the whole IMMUNOLOGY outer group # nolintr
adlb_no_ALT_LH <- dplyr::filter(adlb, PARAMCD != "ALT" | ANRIND == "NORMAL") # normal level and no any abnormal level for ALT # nolintr
adlb_no_ALT_NH <- dplyr::filter(adlb, PARAMCD != "ALT" | ANRIND == "LOW") # normal level and no any abnormal level for ALT # nolintr
...
# Test it out.
s_count_abnormal(adlb %>% dplyr::filter(PARAMCD == "ALT") %>% droplevels(), .var = "ANRIND")
s_count_abnormal(adlb %>% dplyr::filter(PARAMCD == "CRP"), .var = "ANRIND")
# switch order of HIGH and LOW
s_count_abnormal(adlb %>% dplyr::filter(PARAMCD == "CRP"), .var = "ANRIND", abnormal = list(high = "HIGH", low = "LOW"))
...
a_fun(adlb %>% dplyr::filter(PARAMCD == "ALT") %>% droplevels(), .var = "ANRIND")
a_fun(adlb %>% dplyr::filter(PARAMCD == "CRP"), .var = "ANRIND")
...
map_no_ALT <- dplyr::filter(map, PARAMCD != "ALT") # nolint
map_no_ALT
