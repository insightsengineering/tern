...
s_count_abnormal <- function(df,
                             ...
                             variables = list(baseline = "BNRIND"),
  ...
  count_abnormal_single <- function(abn_name, abn) {
    ...
    if (exclude_base_abn) {
      denom <- !(df[[variables$baseline]] %in% abn)
    } else {
      denom <- rep(TRUE, nrow(df))
    }
    ...
    num <- (df[[.var]] %in% abn) & denom

    with_label(c(num = sum(num), denom = sum(denom)), abn_name)
  }

  # This will define the abnormal levels theoretically possible for a specific lab parameter
  # within a split level of a layout.
  abnormal_lev <- sapply(abnormal, intersect, levels(df[[.var]]))
