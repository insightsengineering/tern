...
h_adlb_worsen <- function(
  ...
  adlb_f <- adlb[position_satisfy_filters, ] %>%
    select(tidyselect::all_of(c("USUBJID", "ARMCD", "AVISIT", "PARAMCD", "ATOXGR", "BTOXGR", names(worst_flag), direction_var))) # nolint
  ...
  var_labels(out) <- var_labels(adlb_out_l)
  out
}
...
h_worsen_counter <- function(df, id, .var, baseline_var, direction_var) {
  ...
  names(by_grade) <- as.character(seq_len(by_grade))
