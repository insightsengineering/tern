...
```{r}
load("~/NEST/tern/design/design_ratet01_azpro.RData")
covars <- c("sex", "age75", "admit", "hospital")
anl <- azpro %>% 
  mutate(
    los_f = factor(los),
    procedure = factor(procedure, levels = c(0, 1), labels = c("PTCA", "CAGB"))
  )

lyt <- basic_table() %>%
  split_cols_by("procedure", ref_group = "PTCA") %>%
  add_colcounts() %>% 
  summarize_vars(
    "los_f",
    var_labels = "Number of days in hospital",
    .stats = c("count_fraction")
  ) %>%
  summarize_glm_count(
    vars = "los",
    variables = list(arm = "procedure", offset = NULL, covariates = NULL),
    conf_level = 0.95,
...
-   Perhaps it would be better to re-factor the code so that model fitting
is done separately from the table generation, similar to how Cox regression,
logistic regression, and MMRM currently work.
...
summarize_glm_rate <- function(...){...} ## Treatment arm rates
summarize_glm_rate_ratio <- function(...){...} ## Rate ratio
