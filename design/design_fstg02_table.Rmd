...
h_survtime_df <- function(tte, is_event, arm) {
  checkmate::assert_numeric(tte, min.len = 1, any.missing = FALSE)
  checkmate::assert_logical(is_event, min.len - 1, any.missing = FALSE)
  assertthat::assert_that(
    is_valid_factor(arm),
...
```{r, lyt}
tabulate_tte_subgroups <- function(lyt,
                                   vars,
