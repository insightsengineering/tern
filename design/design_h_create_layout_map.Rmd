...
- LBT04 follow the rule that at least one observation with ANRLO > 0 for low direction and at least one observation with
ANRHI is not missing for high direction.
...
h_map_with_count_abnormal <- function(
  ...
  if (method == "default") {
  ...
  } else if (method == "both") { # both method will have both directions for all observed PARAMs like in VST02ß
    ...
    map <- do.call("rbind", replicate(length(anl_variable_levels), map, simplify = FALSE))
