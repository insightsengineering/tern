...
h_map_with_count_abnormal <- function(
  ...
  if (method == "default") {
    ...
    map_normal <- unique(subset(map, select = split_rows_varibles))
    ...
    # both method will have both directions for all observed PARAMs like in VST02ß
...
df <- data.frame(
  ...
  AVISIT = c(rep("WEEK 1", 2), rep("WEEK 2", 2), rep("WEEK 1", 2), rep("WEEK 2", 2), rep("WEEK 1", 2), rep("WEEK 2", 2)),
...
```{r}
ADLB <- haven::read_sas("../adlb.sas7bdat") %>% df_explicit_na()
