...
adsl_f <- adsl %>%
  ...
  dplyr::mutate(
    STSTFL = dplyr::case_when(is.na(RANDDT) ~ "N", # derive flag for "Started Study",
                     TRUE ~ "Y"),
    COMPSTUD = dplyr::case_when(EOSSTT == "COMPLETED" ~ "Y", # derive flag for "Completed Study"
                         TRUE ~ "N"),
    DISCSTUD = dplyr::case_when(EOSSTT == "DISCONTINUED" ~ "Y", # derive flag for "Discontinued study"
                         TRUE ~ "N"),
    AGEGRP = dplyr::case_when(AGE < 65 ~ "< 65 yrs",
                       AGE >= 65 ~ ">= 65 yrs"),
...
adae_serious <- adae %>% dplyr::filter(AESER == "Y", SAFFL == "Y") #Filtering for only serious AEs
adae_nonser <- adae %>% dplyr::filter(AESER != "Y", SAFFL == "Y")#Filtering for only non-serious AEs
#nolint end
```
