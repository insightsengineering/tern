...
adlb <- df_explicit_na(adlb)
  ...
  qntls <- adlb %>%
    ...
    dplyr::summarise(as_tibble(t(quantile(AVAL, probs = c(0.1, 0.9)))), .groups = "drop_last") %>%
    dplyr::rename(q1 = 2, q2 = 3)
  ...
  adlb <- adlb %>%
    ...
    AVALCAT1 = factor(dplyr::case_when(
      ANRIND %in% c("HIGH HIGH", "LOW LOW") ~
        sample(x = avalcat1, size = dplyr::n(), replace = TRUE, prob = c(0.3, 0.6, 0.1)),
      TRUE ~ ""),
      levels = c("", avalcat1)
    ),
    PARCAT2 = factor(ifelse(ANRIND %in% c("HIGH HIGH", "LOW LOW"), "LS",
                            sample(c("SI", "CV", "LS"), size = dplyr::n(), replace = TRUE)
    )
    )) %>%
    dplyr::select(-q1, -q2)

  #Preprocessing steps
  adlb_f <- adlb %>% dplyr::filter(ONTRTFL == "Y" & PARCAT2 == "LS" & SAFFL == "Y" & !is.na(AVAL))
...
# will add an additional preprocessing step to derive a new variable
# that is going to be used as the split variable instead of ANRIND,
# to have a correct split labels
adlb_f <- adlb_f %>%
...
```{r}
# we do not have to add AVALCAT1 analysis variable in the map.
map <- unique(
adlb_f[adlb_f$abn_dir %in% c("Low", "High") & adlb_f$AVALCAT1 != "", c("PARAM", "abn_dir")]
) %>%
  lapply(as.character) %>%
  as.data.frame() %>%
  dplyr::arrange(PARAM, !dplyr::desc(abn_dir))
...
# If we want to remove the 0 cases of `Single, not last` and `Last or replicated`
# we can achieve this with `trim_levels_in_group`

table_in_group <- build_table(lyt = lyt_in_group, df = adlb_f %>% dplyr::filter(!AVALCAT1 %in% c("SINGLE")) %>% droplevels())
table_in_group
...
adlb_f_no_ALT <- adlb_f %>% #nolint
  dplyr::filter(PARAM != "Alanine Aminotransferase Measurement") %>%
  droplevels()
...
```{r}
adlb_f2 <- adlb_f %>%
  dplyr::filter(!(ARMCD == "ARM A" & PARAMCD == "ALT")) %>%
  droplevels()
