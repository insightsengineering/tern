...
We want to provide functions to enable Subgroup Treatment Effect Pattern (STEP) graphs for survival and response outcomes. See [https://pages.github.roche.com/Rpackages/rBiomarker/dev/articles/Showcases.html#survival-outcome](here) for a mockup. Generally we will follow the `rBiomarker` package function `PlotSTEP`, see [https://github.roche.com/Rpackages/rBiomarker/blob/master/R/PlotSTEP.R](code here):

> This function computes the treatment effect as a function of a biomarker variable and
...
We adapt a bit the window function [WindowSel](https://github.roche.com/Rpackages/rBiomarker/blob/c4f1e72b4b265c421226ad3a5279ce24ce984d01/R/PlotSTEP.R#L496) from `rBiomarker`.
...
It is not clear if the [PredictTE](https://github.roche.com/Rpackages/rBiomarker/blob/c4f1e72b4b265c421226ad3a5279ce24ce984d01/R/PlotSTEP.R#L273) function from `rBiomarker` is doing the correct thing for the polynomial biomarker interaction case. Therefore we tried to understand this from the provided `predict()` method in R from scratch.
...
g_step <- function(step_matrix) {
  assert_that(is(step_matrix, "step"))
  ...
  ggplot(plot_dat) +
    ...
    geom_line(aes_string(x = "x", y = "y", color = "type", linetype = "type", group = "bound"))
}
