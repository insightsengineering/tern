...
adcm_path2 <- adcm_path1 %>%
  dplyr::filter(CMDECOD %in% c("medname A_1/3", "medname B_2/4", "medname B_3/4", "medname C_1/2")) %>%
  dplyr::mutate(
    ATC1 = paste(ATC1, "p2"),
    ATC2 = paste(ATC2, "p2"),
    ATC3 = paste(ATC3, "p2"),
...
adcm_path3 <- adcm_path2 %>%
  dplyr::filter(CMDECOD %in% c("medname B_2/4", "medname C_1/2")) %>%
  dplyr::mutate(
    ATC1 = paste(ATC1, "p3"),
    ATC2 = paste(ATC2, "p3"),
    ATC3 = paste(ATC3, "p3"),
...
adcm <- adcm %>%
  dplyr::group_by(STUDYID, USUBJID, CMDECOD) %>%
  dplyr::slice(1) %>%
  ungroup
```
