...
s_count_patients_exposure_in_cols <- function(df, # nolint
  ...
  n_patients <- formatable::with_label(
    s_num_patients_content(df = df, .N_col, .var = id, labelstr = "")$unique,
    row_label
  )

  sum_exposure <- formatable::with_label(sum(df[[.var]]), row_label)
...
``` {r}
colfuns <- list(
  function(x, .N_col, labelstr) {
    in_rows(" " = rcell( # nolint
      c(length(unique(x)), length(unique(x)) / .N_col),
      format = "xx (xx.x%)"
    ))
  },
