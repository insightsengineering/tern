...
pth <- "/opt/bee/analyses/stream2/master/2.11/stream2/doc/examples/molecule/project/efficacy/task01/outdata_vad/"
...
s_count_patients_exposure_in_cols <- function(df, 
                                              .var = "AVAL" ,
                                              id = "USUBJID",
                                              labelstr = "",
                                              .N_col,
...
``` {r}
colfuns <- list(function(x, .N_col, labelstr) in_rows( " " = rcell(c(length(unique(x)), length(unique(x))/.N_col), format = "xx (xx.x%)")),
