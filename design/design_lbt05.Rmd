...
adlb_f <-
  ...
  dplyr::group_by(USUBJID, PARAMCD, DIRECTION) %>%
  dplyr::mutate(MAX_AVALCAT1 = max(AVALCAT1_NUM)) %>%
  dplyr::ungroup() %>%
  dplyr::mutate(
    AVALCAT1_NUM = ifelse(AVALCAT1_NUM == 1 &
                            AVALCAT1_NUM < MAX_AVALCAT1, 0, AVALCAT1_NUM),
    AVALCAT1_FL = dplyr::case_when(
...
adsl_f <- adsl %>% dplyr::filter(SAFFL == "Y")
