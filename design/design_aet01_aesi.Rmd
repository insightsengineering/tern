...
adae <- dplyr::filter(adae, (!is.na(AEBODSYS) | !is.na(AEDECOD)) & ANL01FL == "Y" & SAFFL == "Y")
...
not_resolved <- adae %>%
  dplyr::filter(AEOUT != "RECOVERED/RESOLVED" & AEOUT != "FATAL") %>%
  dplyr::select(unique("USUBJID")) %>%
  dplyr::mutate(
    NOT_RESOLVED = "Y"
  )
adae <- adae %>%
  dplyr::left_join(not_resolved, by = c("USUBJID")) %>%
  dplyr::mutate(
...
adae <- adae %>%
  dplyr::left_join(df_max, by = c("USUBJID")) %>%
  dplyr::mutate(
