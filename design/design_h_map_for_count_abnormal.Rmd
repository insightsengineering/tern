...
```{r}
df <- df %>% dplyr::mutate(ANRIND = ifelse(PARAM == "ALT", "NORMAL", as.character(ANRIND)))
df$ANRIND <- factor(df$ANRIND, levels = c("LOW", "HIGH", "NORMAL")) #nolint
...
adsl <- ADSL %>% df_explicit_na() %>% dplyr::filter(SAFFL == "Y")
adlb_f <- ADLB %>% df_explicit_na() %>% dplyr::filter(ONTRTFL == "Y" & ATOXGR != "<Missing>" & PARCAT2 == "SI") %>%
  dplyr::mutate(ATOXGR = factor(ATOXGR, levels = c("-4", "4", "-1", "-2", "-3", "0", "1", "2", "3", "<Missing>")),
...
build_table(lyt, adlb_f %>% dplyr::filter(LBCAT != "<Missing>"), alt_counts_df = adsl)
...
build_table(lyt, adlb_f_2 %>% dplyr::filter(LBCAT != "<Missing>"), alt_counts_df = adsl)
```

replace stream data with scda so that knit will work
```{r}
ADLB <- haven::read_sas("../adlb.sas7bdat") %>% df_explicit_na() #nolint
adlb_f <- ADLB %>% dplyr::filter(ONTRTFL == "Y" & ANRIND != "<Missing>" & PARCAT2 == "SI")

h_map_with_count_abnormal(
  df = adlb_f %>% dplyr::filter(LBCAT != "<Missing>"),
...
```{r}
adlb_f_noALTSI <- adlb_f %>% dplyr::filter(PARAMCD != "ALTSI")
build_table(lyt, adlb_f_noALTSI %>% dplyr::filter(LBCAT != "<Missing>"), alt_counts_df = adsl)
```
...
build_table(lyt_test, adlb_f_noALTSI %>% dplyr::filter(LBCAT != "<Missing>"), alt_counts_df = adsl)
```
