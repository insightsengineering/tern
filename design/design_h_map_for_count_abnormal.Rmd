...
h_map_for_count_abnormal <- function(
  ...
  if (method == "default") {
    ...
    ext_ncictc4[ext_ncictc4[["RESULT"]] %in% c("1", "2", "3", "4"), ][[variables$anl]] <- "HIGH"
    ext_ncictc4[ext_ncictc4[["RESULT"]] %in% c("-1", "-2", "-3", "-4"), ][[variables$anl]] <- "LOW"
    ext_ncictc4[ext_ncictc4[["RESULT"]] %in% c("0"), ][[variables$anl]] <- "NORMAL"
...
For lbt15, use stream real data to test
```{r}
ADLB <- haven::read_sas("../data/adlb.sas7bdat") %>% df_explicit_na() #nolint
